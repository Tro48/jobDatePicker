...
test('правка без смены отрезком не считается', () => {
  // Две заметки подряд — не отпуск. Без проверки они склеились бы в «отрезок»
  // из двух undefined, и карточка дня предложила бы задать его длину.
  const overrides = new Map<IsoDate, DayOverride>([
    ['2026-09-07', { date: '2026-09-07', note: 'занёс справку' }],
    ['2026-09-08', { date: '2026-09-08', note: 'отдал сменщику' }],
  ]);
