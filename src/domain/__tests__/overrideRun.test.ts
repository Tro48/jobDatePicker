...
test('правка без смены отрезком не считается', () => {
  // Два дня с правлеными часами — не отпуск. Без проверки они склеились бы в
  // «отрезок» из двух undefined, и карточка дня предложила бы задать длину.
  const overrides = new Map<IsoDate, DayOverride>([
    ['2026-09-07', { date: '2026-09-07', workedMinutesOverride: 300 }],
    ['2026-09-08', { date: '2026-09-08', workedMinutesOverride: 360 }],
  ]);

  assert.equal(findOverrideRun(overrides, '2026-09-07'), null);
  assert.equal(findOverrideRun(overrides, '2026-09-08'), null);
});
