 ...
 * человек сам нажал значок QR в окне нового графика, а не при первом запуске
 * приложения.
        ...
        <Card title="Наведи на код">
          ...
          {foreign ? (
            <AppText variant="body" accessibilityLiveRegion="polite">
              Этот код не из приложения. Нужен тот, что показывает значок QR в карточке «График» на
              другом телефоне.
            </AppText>
          ) : (
            <AppText variant="body" tone="muted">
              На другом телефоне код открывается значком QR в карточке «График». После распознавания
              здесь откроется предпросмотр — до него ничего не изменится.
            </AppText>
          )}
        </Card>
      </ScrollView>
