 ...
 *
 * Значения здесь — то, с чего палитра начинается: каждая пара «заливка +
 * подпись» проверена скриптом контраста в обеих темах. Поверх них человек
 * может положить свои цвета («Настройки → Оформление → Цвета»), и тогда
 * название оттенка расходится с тем, что видно в клетке, — поэтому цвет в
 * списке выбора показан кружком, а название идёт озвучке подписью к нему.
 * Заданный руками цвет проверяется тем же порогом контраста, но прямо на
 * экране: скрипт в CI до чужого телефона не дотягивается.
 *
